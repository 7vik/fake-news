...
def article_catch(url_list, j):
    ...
    for i, item in enumerate(url_list):
        ...
        title = Soup.find('div', {'class': 'row title-article'}).text
        time = Soup.find('li', {'class': 'date-published'}).text
        pictinfo = Soup.find('div', {'class': 'featured-image'}).get('style')
        image = str(re.search(r'http.+?jpg', pictinfo).group())
        print(image)
...
url_base = 'http://www.cracked.com/previous-featured-contents/?page='
for i in range(1, 3):
